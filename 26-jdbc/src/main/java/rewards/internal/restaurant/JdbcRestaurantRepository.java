...
public class JdbcRestaurantRepository implements RestaurantRepository {
   ...
   * {@link BenefitAvailabilityPolicy} object. The key column is 'BENEFIT_AVAILABILITY_POLICY',
   * which is a discriminator column containing a string code that identifies the type of policy.
   * Currently supported types are: 'A' for 'always available' and 'N' for 'never available'.
   * <p>
   * More types could be added easily by enhancing this method. For example, 'W' for 'Weekdays only'
   * or 'M' for 'Max Rewards per Month'. Some of these types might require additional database
   * column values to be configured, for example a 'MAX_REWARDS_PER_MONTH' data column.
   *
