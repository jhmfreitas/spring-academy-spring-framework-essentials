...
public class AccountController {
   ...
   * <p>
   * Suppose we have just received an incoming URL of, say, http://localhost:8080/accounts and
   * resourceId is "1111". Then the URL of the new resource will be
  ...
  private ResponseEntity<Void> entityWithLocation(Object resourceId) {
    ...
    URI builder = ServletUriComponentsBuilder.fromCurrentRequestUri().path("/{resourceId}")
        .buildAndExpand(resourceId).toUri();

    return ResponseEntity.created(builder).build(); // Return something other than null
  }
  ...
  /**
   * Adds a Beneficiary with the given name to the Account with the given id, setting its URL as the
  ...
  public void removeBeneficiary(@PathVariable long accountId,
    ...
    // exception.
    if (account.getBeneficiaries().size() != 1 && (!b.getAllocationPercentage()
  ...
  public void handleConflict(Exception ex) {
    logger.error("Data integrity violation is: ", ex);
  }
  ...
  /**
   * Finds the Account with the given id, throwing an IllegalArgumentException if there is no such
   * Account.
   */
